...
-export([start/0, start_link/0, put_message/2, get_all_messages/1, get_message/2, delete_message/2, pass_messages/2]).
...
handle_call({put, Handle, Message}, _FromPid, State) when Message#mqtt.id /= undefined ->
  Response = dets:insert(State#store.table, {Handle, Message#mqtt.id, Message}),
...
handle_call({delete, Handle, MessageId}, _FromPid, State) ->
  Response = dets:match_delete(State#store.table, {Handle, MessageId, '_'}), 
